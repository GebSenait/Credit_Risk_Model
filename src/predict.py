...
class CreditRiskPredictor:
    ...
    def _load_model(self):
        ...
        if feature_path.exists():
            ...
            logger.warning("Feature names file not found. Predictions may fail if feature mismatch occurs.")
    ...
    def predict_risk_score(self, data: Union[pd.DataFrame, Dict]) -> Dict[str, Union[float, int]]:
        """
        ...
        default_prob = probabilities[:, 1] if len(probabilities.shape) > 1 else probabilities[1]
        ...
        results = {
            ...
            'default_probability': float(default_prob[0]) if len(default_prob) == 1 else default_prob.tolist(),
